...
async fn graphql_variables_keep_float_type_after_integer_value() {
    ...
    let mutation_name = state
        ...
        .iter()
        .find(|field| {
            field.table_name == table_name && field.mutation_type == MutationType::Insert
        })
    ...
    let update_name = state
        ...
        .iter()
        .find(|field| {
            field.table_name == table_name && field.mutation_type == MutationType::Update
        })
    ...
    let count_name = state
        ...
        .clone();
    let count_query =
        format!("query Count($filter: JSON) {{ {count_name}(filter: $filter) }}");
    ...
    let stored_values: Vec<f64> = sqlx::query_scalar(&format!(
        "SELECT value FROM {qualified_table} ORDER BY id"
    ))
    .fetch_all(&pool)
    .await
    ...
    let integer_count = state
        .schema
        .execute(add_context(
            Request::new(count_query.clone()).variables(Variables::from_json(json!({
                "filter": { "value": { "eq": 12 } }
            }))),
        ))
        .await;
    let fractional_count = state
        .schema
        .execute(add_context(
            Request::new(count_query).variables(Variables::from_json(json!({
    ...
    let fractional_update = state
        .schema
        .execute(add_context(
            Request::new(update_mutation).variables(Variables::from_json(json!({
                "where": { "id": { "eq": 1 } },
                "set": { "value": 20.5 }
            }))),
        ))
        .await;
    let updated_value: f64 = sqlx::query_scalar(&format!(
        "SELECT value FROM {qualified_table} WHERE id = 1"
    ))
    .fetch_one(&pool)
    .await
    ...
    let null_response = state
        .schema
        .execute(add_context(
            Request::new(mutation.clone()).variables(Variables::from_json(json!({
    ...
    assert!(integer_count.errors.is_empty(), "integer filter failed: {:?}", integer_count.errors);
    assert!(fractional_count.errors.is_empty(), "fractional filter failed: {:?}", fractional_count.errors);
    assert_eq!(serde_json::to_value(integer_count.data).unwrap()[&count_name], 1);
    assert_eq!(serde_json::to_value(fractional_count.data).unwrap()[&count_name], 1);
    assert!(integer_update.errors.is_empty(), "integer update failed: {:?}", integer_update.errors);
    assert!(fractional_update.errors.is_empty(), "fractional update failed: {:?}", fractional_update.errors);
    assert_eq!(updated_value, 20.5);
    assert!(null_response.errors.is_empty(), "null insert failed: {:?}", null_response.errors);
    assert!(value_after_null.errors.is_empty(), "post-null insert failed: {:?}", value_after_null.errors);
